...
// ---------------------------------------------------------------------------
// Fonction qui calcule les valeurs de la fractale de Mandelbrot pour la zone
// [minX, maxX] x [minY, maxY] et renvoie un tableau de pixels RGBA.
// ---------------------------------------------------------------------------
std::vector<sf::Uint8> computeMandelbrot(unsigned width, unsigned height,
                                        ...
                                        unsigned maxIter)
{
    ...
    for (unsigned py = 0; py < height; ++py) {
        for (unsigned px = 0; px < width; ++px) {
            // Convertir (px,py) en coordonnées complexes
...
int main()
{
    ...
    // Calcul initial de la fractale
    std::vector<sf::Uint8> pixelData = computeMandelbrot(WIDTH, HEIGHT,
                                                         minX, maxX,
                                                         minY, maxY,
                                                         MAX_ITER);
            ...
            if (event.type == sf::Event::MouseButtonReleased) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    if (std::abs(x2 - x1) > 5 && std::abs(y2 - y1) > 5) {
                        ...
                        pixelData = computeMandelbrot(WIDTH, HEIGHT,
                                                      minX, maxX,
                                                      minY, maxY,
                                                      MAX_ITER);
